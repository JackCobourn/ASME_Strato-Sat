...
/* Store the file for writing to the SD card */
File dataFile; 
...
void buildSensorData(){
  ...
  writeFile(x,dataFile);
  writeFile(y,dataFile);
  writeFile(z,dataFile);
}
...
void writeFile(double dataString, File dataFile){
  // if the file is available, write to it:
  if (dataFile) {
    dataFile.print(dataString);
    dataFile.close();
...
void setup(void) 
{
  ...
  if(!accel.begin())
  {
   ...
   if(IIC_Read(WHO_AM_I) == 196) 
    ...
    Serial.println("Ooops, no MPL3115A2 detected .... Check your wiring!");
  ...
  /* Save some basic information on this sensor */  
  writeFile(printHeaders(),dataFile);
  
}

void loop(void) 
{
  writeFile(2.1,dataFile);
  writeNewLine(dataFile);
  delay(500);
}
